...
scirs2-optimize = { version = "0.1.0-beta.2", features = ["parallel", "simd"] }
```
...
1. **Enable Parallelization**:
   ```toml
   scirs2_optimize = { version = "0.1.0-beta.2", features = ["parallel"] }
   ```

2. **Use SIMD Operations**:
   ```toml
   scirs2_optimize = { version = "0.1.0-beta.2", features = ["simd"] }
   ```
