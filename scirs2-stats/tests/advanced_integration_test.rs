...
fn test_largedata_integration() {
    ...
    let mean_result = mean(&largedata.view()).expect("Test: operation failed");
    assert!((mean_result - 500.5).abs() < 1e-10);

    let var_result = var(&largedata.view(), 1, None).expect("Test: operation failed");
    assert!(var_result > 0.0);

    let std_result = std(&largedata.view(), 1, None).expect("Test: operation failed");
    assert!(std_result > 0.0);
}
