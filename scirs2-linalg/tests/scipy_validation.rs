...
fn test_determinant_against_scipy() {
    ...
    let actual_det = det(&a.view(), None).expect("Test: operation failed");
...
fn test_inverse_against_scipy() {
    ...
    let actual_inv = inv(&a.view(), None).expect("Test: operation failed");
...
fn test_lu_decomposition_against_scipy() {
    ...
    let (p, l, u) = lu(&a.view(), None).expect("Test: operation failed");
...
fn test_qr_decomposition_against_scipy() {
    ...
    let (q, r) = qr(&a.view(), None).expect("Test: operation failed");
...
fn test_cholesky_against_scipy() {
    ...
    let l = cholesky(&a.view(), None).expect("Test: operation failed");
...
fn test_norm_against_scipy() {
    ...
    let actual_frobenius = matrix_norm(&a.view(), "fro", None).expect("Test: operation failed");
    ...
    let actual_1norm = matrix_norm(&a.view(), "1", None).expect("Test: operation failed");
    assert_abs_diff_eq!(actual_1norm, expected_1norm, epsilon = TOLERANCE);

    // inf-norm (max absolute row sum)
    // SciPy result: 24.0
    let expected_infnorm = 24.0;
    let actual_infnorm = matrix_norm(&a.view(), "inf", None).expect("Test: operation failed");
...
fn test_solve_against_scipy() {
    ...
    let actual_x = solve(&a.view(), &b.view(), None).expect("Test: operation failed");
...
fn test_edge_cases() {
    ...
    let det_result = det(&singular.view(), None).expect("Test: operation failed");
    ...
    let identity_det = det(&identity.view(), None).expect("Test: operation failed");
    assert_abs_diff_eq!(identity_det, 1.0, epsilon = TOLERANCE);

    // Inverse of identity should be identity
    let identity_inv = inv(&identity.view(), None).expect("Test: operation failed");
...
fn test_numerical_stability() {
    ...
    let det_result = det(&a.view(), None).expect("Test: operation failed");
    assert!((det_result - 0.000001).abs() < 1e-10);

    // Test with very small values
    let small = array![[1e-10_f64, 2e-10], [3e-10, 4e-10]];

    let small_det = det(&small.view(), None).expect("Test: operation failed");
