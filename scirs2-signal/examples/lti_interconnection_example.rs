...
fn main() {
    ...
    // G1(s) = 5 / (s + 2)
    let g1 = tf(vec![5.0], vec![1.0, 2.0], None).expect("Test: operation failed");
    println!("G1(s) = 5 / (s + 2)");

    // G2(s) = 1 / (s + 5)
    let g2 = tf(vec![1.0], vec![1.0, 5.0], None).expect("Test: operation failed");
    println!("G2(s) = 1 / (s + 5)");

    // Series connection: H(s) = G2(s) * G1(s)
    let series_sys = series(&g1, &g2).expect("Test: operation failed");
    ...
    // G1(s) = 3 / (s + 1)
    let g1_par = tf(vec![3.0], vec![1.0, 1.0], None).expect("Test: operation failed");
    println!("G1(s) = 3 / (s + 1)");

    // G2(s) = 2 / (s + 4)
    let g2_par = tf(vec![2.0], vec![1.0, 4.0], None).expect("Test: operation failed");
    println!("G2(s) = 2 / (s + 4)");

    // Parallel connection: H(s) = G1(s) + G2(s)
    let parallel_sys = parallel(&g1_par, &g2_par).expect("Test: operation failed");
    ...
    // Plant: G(s) = 10 / (s(s + 1))
    let plant = tf(vec![10.0], vec![1.0, 1.0, 0.0], None).expect("Test: operation failed");
    println!("Plant G(s) = 10 / (s(s + 1))");

    // Unity feedback (negative feedback)
    let closed_loop = feedback(&plant, None, 1).expect("Test: operation failed");
    ...
    // Plant: G(s) = 1 / ((s + 1)(s + 2))
    let plant_pid = tf(vec![1.0], vec![1.0, 3.0, 2.0], None).expect("Test: operation failed");
    println!("Plant G(s) = 1 / ((s + 1)(s + 2))");

    // PID Controller: C(s) = Kp + Ki/s + Kd*s = (Kd*s^2 + Kp*s + Ki) / s
    // Let's use Kp = 10, Ki = 5, Kd = 2
    let controller =
        tf(vec![2.0, 10.0, 5.0], vec![1.0, 0.0], None).expect("Test: operation failed");
    println!("PID Controller C(s) = (2s^2 + 10s + 5) / s");

    // Series connection of controller and plant
    let forward_path = series(&controller, &plant_pid).expect("Test: operation failed");
    println!("Forward path = C(s) * G(s)");

    // Closed-loop with unity feedback
    let pid_closed_loop = feedback(&forward_path, None, 1).expect("Test: operation failed");
    ...
    let sens = sensitivity(&plant, None).expect("Test: operation failed");
    let comp_sens = complementary_sensitivity(&plant, None).expect("Test: operation failed");
    ...
    let sum_st = parallel(&sens, &comp_sens).expect("Test: operation failed");
    ...
    for &freq in &freqs {
        let open_loop_resp = plant
            .frequency_response(&[freq])
            .expect("Test: operation failed");
        let closed_loop_resp = closed_loop
            .frequency_response(&[freq])
            .expect("Test: operation failed");
    ...
    let (w, mag, phase) = bode(&closed_loop, Some(&bode_freqs)).expect("Test: operation failed");
    ...
    let prefilter = tf(vec![1.0], vec![0.1, 1.0], None).expect("Test: operation failed");
    println!("Prefilter F(s) = 1 / (0.1s + 1)");

    // Simple proportional controller: C(s) = 5
    let prop_controller = tf(vec![5.0], vec![1.0], None).expect("Test: operation failed");
    println!("Controller C(s) = 5");

    // Plant: G(s) = 1 / (s + 1)
    let simple_plant = tf(vec![1.0], vec![1.0, 1.0], None).expect("Test: operation failed");
    println!("Plant G(s) = 1 / (s + 1)");

    // Build the system step by step
    let controller_plant = series(&prop_controller, &simple_plant).expect("Test: operation failed");
    let closed_inner = feedback(&controller_plant, None, 1).expect("Test: operation failed");
    let complete_system = series(&prefilter, &closed_inner).expect("Test: operation failed");
