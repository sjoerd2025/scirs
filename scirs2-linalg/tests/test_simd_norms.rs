...
fn test_simd_norm_l1_f64_basic() {
    ...
    let norm = vector_norm_simd(&x.view(), 1).unwrap();
...
fn test_simd_norm_l2_f64_basic() {
    ...
    let norm = vector_norm_simd(&x.view(), 2).unwrap();
...
fn test_simd_norm_linf_f64_basic() {
    ...
    let norm = vector_norm_simd(&x.view(), usize::MAX).unwrap();
...
fn test_simd_norm_relationship() {
    ...
    let norm_l1 = vector_norm_simd(&x.view(), 1).unwrap();
    let norm_l2 = vector_norm_simd(&x.view(), 2).unwrap();
    let norm_linf = vector_norm_simd(&x.view(), usize::MAX).unwrap();
...
fn test_simd_frobenius_norm_f64_basic() {
    ...
    let norm = matrix_norm_simd(&a.view(), "fro", None).unwrap();
...
fn test_simd_frobenius_vs_scalar() {
    ...
    let norm_simd = matrix_norm_simd(&a.view(), "fro", None).unwrap();
    let norm_scalar = matrix_norm(&a.view(), "fro", None).unwrap();
